...
public class DoubleInterval implements Comparable<DoubleInterval> {
	...
	public DoubleInterval(IntegerInterval iInterval) {
		this.min = iInterval.getDoubleMin();
		this.max = iInterval.getDoubleMax();
	}
	...
	public DoubleInterval clone() {
		DoubleInterval result = new DoubleInterval();
		result.min = this.min;
		result.max = this.max;
		return result;
	}
